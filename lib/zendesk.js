...
Zendesk.prototype.deleteUser = function(user,callback){
    var options = {
        path: this.path + "/users/"+user+".json"
    };
    this.httpsRequest("DELETE",options,callback);
};
...
Zendesk.prototype.createOrg = function(org,callback){
    var options = {
...
Zendesk.prototype.getUserByName = function(name,callback){
    var query = this.setQueryParams({
        query: {
            type: "user",
        }
    ...
    var options = {
        path: this.path + "/search.json"+ query + "%20" + name
    };
    console.log(options);
    this.httpsRequest("GET",options,callback);
};
